...
contract GatewayManager is iERC173, iGatewayManager {
    ...
    function randomGateways(
        ...
        string memory _path,
        uint256 seed
    ) public view returns (string[] memory gateways) {
        ...
        if (_recordhash.length == 0) {
            ...
            while (gateLen > 0) {
                --gateLen;
                gateways[gateLen] = string.concat(
                    string("https://"),
                    Web2Gateways[gateLen],
                    _path,
                    ".json?t={data}"
                );
            }
            ...
            _recordhash[0] == bytes1("h") &&
            iGatewayManager(this).isWeb2(_recordhash)
        ) {
            /// Trusted web2 gateway set by the owner
            gateways = new string[](1);
            gateways[0] = string.concat(
                string(_recordhash),
                _path,
                ".json?t={data}"
            );
        ...
        unchecked {
            ...
            while (i < len) {
                seed = uint256(
                    keccak256(abi.encodePacked(block.number * i, seed))
                );
                gateways[i++] = string.concat(
                    "https://",
                    Web3Gateways[seed % gLen],
                    _fullPath
                );
            }
            gateways[len] = string.concat(
                "https://",
                Web2Gateways[0],
                _fullPath
            ); // Fallback
    ...
    function toChecksumAddress(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(bytesToHexString(_buffer, 0))
        );
        uint256 high;
        uint256 low;
